...
const RevenueExpenseChart: React.FC<Props> = ({ year, month }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const transformed = filtered.map((item: DayTransaction) => ({
          day: item.dayOfMonth,
          booking: item.booking,
          memberShip: item.memberShip,
          spending: -Math.abs(item.spending),
          refund: -Math.abs(item.refund),
          voucher: -Math.abs(item.voucher),
          deposit: item.deposit,
        }));

        setData(transformed);
      } catch (error) {
        console.error("Lỗi khi fetch dữ liệu doanh thu theo ngày:", error);
      } finally {
        setIsLoading(false);
      }
    };
          ...
          >
            ...
            <Tooltip
              formatter={(value, name) => [
                ...
                displayNames[name as string] || name,
              ]}
              contentStyle={{ fontSize: "13px" }}
              labelStyle={{ color: "#000" }}
            />
            <Legend
              formatter={(value: string) => displayNames[value] || value}
              wrapperStyle={{ fontSize: "12px" }}
            />
            <Bar dataKey="deposit" name="Nạp tiền" fill="#34d399" />
            <Bar dataKey="booking" name="Đặt bàn" fill="#de4ad4" />
            <Bar dataKey="memberShip" name="Thành viên" fill="#60a5fa" />
